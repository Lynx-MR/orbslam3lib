...
FrameAHB Tracking::GrabImageStereoAHB(AHardwareBuffer* imagesBuffer, const double &timestamp, string filename){
    // We suppose we have only a gray monochrome image...

    if (mSensor == System::STEREO && !mpCamera2) {
        std::lock_guard<std::mutex> lock(mMutexDualProc);
        FrameAHB frame = FrameAHB(imagesBuffer, timestamp, mpORBextractorLeft, mpORBextractorRight,
                                 mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera);

        mNextDualProcFrame = frame;
        return frame;
    }
    else if(mSensor == System::STEREO && mpCamera2){
        FrameAHB frame = FrameAHB(imagesBuffer,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
...
Sophus::SE3f Tracking::GrabImageStereoAHBFinish(FrameAHB frameAHB){
    {
        std::lock_guard<std::mutex> lock(mMutexDualProc);
        mCurrentFrame = mNextDualProcFrame;
    }

    if(mSensor == System::STEREO && mpCamera2){
        ((FrameAHB*)&mCurrentFrame)->FinishBuild();
    }
    else{
        std::cout << "Unsupported camera type (finish AHB)" << std::endl;
    }

    mCurrentFrame.mnDataset = mnNumDataset;

#ifdef REGISTER_TIMES
    vdORBExtract_ms.push_back(mCurrentFrameAHB.mTimeORB_Ext);
    vdStereoMatch_ms.push_back(mCurrentFrameAHB.mTimeStereoMatch);
#endif
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    {
        cout << "TRACK_REF_KF: Less than 10 matches!! (" << nmatches << ")" << endl;
        return false;
    }
...
bool Tracking::TrackLocalMap()
{
    ...
    {
        if(mnMatchesInliers<10) //TODO REPLACE BY 15 !!!
        {
            return false;
        }
        else
    ...
    {
        if(mnMatchesInliers<15) //TODO REPLACE BY 30
            return false;
        else
            return true;
    }
...
bool Tracking::Relocalization()
{
    ...
    bool bMatch = false;
    ORBmatcher matcher2(0.7,true); //DEFAULT 0.9

    constexpr int maxIterations = 3;
    int currentIterations = 0;
    while(nCandidates>0 && !bMatch && currentIterations < maxIterations)
    {
        currentIterations++;
