...
{
    //return FastPoseOptimization(pFrame);
    //TEST NEW METHOD AND COMPARE ABOVE WITHOUT WRITING THE RESULTS
    ...
    std::cout << "Found " << nInitialCorrespondences << " correspondences for PoseOptimization" << std::endl;
    ...
    //LYNX MODIFIED FOR FASTER COMPUTE TIME
    const int its[4]={1,2,10,10};   //DEFAULT 10

    int nBad=0;
    for(size_t it=0; it<2; it++) //DEFAULT 4
    {
        Tcw = pFrame->GetPose();
        vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(),Tcw.translation().cast<double>()));

        optimizer.initializeOptimization(0);
        optimizer.optimize(its[it]); //THIS IS THE LINE THAT TAKES MOST OF THE TIME
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale)
{
    ...
    std::unique_ptr<g2o::OptimizationAlgorithmGaussNewton> solver =
            std::make_unique<g2o::OptimizationAlgorithmGaussNewton>(std::move(solver_ptr));
