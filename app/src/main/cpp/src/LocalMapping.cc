...
namespace ORB_SLAM3
{
    ...
    void LocalMapping::CreateNewMapPoints()
    {
        ...
        for(size_t i=0; i<vpNeighKFs.size(); i++)
        {
            ...
            {
                if(mpCurrentKeyFrame == nullptr) continue;
                if(ikp < vMatchedIndices.size()) continue;

                const int &idx1 = vMatchedIndices[ikp].first;
                const int &idx2 = vMatchedIndices[ikp].second;

                if(idx1 >= mpCurrentKeyFrame->mvuRight.size() || idx2 >= pKF2->mvuRight.size())
                    continue;
                ...
                if(pMP)
                {
                    if(!pMP->isBad())
                    {
                        if(!mbMonocular)
                        {
                            ...
                            if(i > pKF->mvDepth.size()) continue;
                        ...
                        if(pMP->Observations()>thObs)
                        {
                            // Lynx Avoid segfault
                            if(pKF == nullptr) continue;
                            if(i > pKF->mvKeys.size()) continue;
                            if(i > pKF->mvKeysUn.size()) continue;
                            if(i > pKF->mvKeysRight.size()) continue;
