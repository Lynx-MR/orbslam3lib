...
{
    //Some copy errors happen randomly concerning Eigen objects, to be able to debug the next crash, we will copy the eigen objects one by one below
...
void Frame::ComputeStereoFishEyeMatches() {
    ...
    std::vector<uint16> indices, dist1, dist2;

    assert(LynxHardwareAccelerator::lynxHardwareAccelerator != nullptr);
    LynxHardwareAccelerator::lynxHardwareAccelerator->BFMatchORB(mnIdMatchingData, stereoDescRight, stereoDescLeft, indices, dist1, dist2);

    for(int i = 0; i < stereoDescLeft.rows; i++){
        //if(dist1[i] < dist2[i] * 0.9f){ //0.7f per default

        constexpr uint16_t max_distance = 70;

        if(dist1[i] == 0){
            //Bugged point
            std::cout << "Bugged point : " << i << " => " << indices[i] << "(" << mvKeys[i + monoLeft].pt.x << ", " << mvKeys[i + monoLeft].pt.y << ") => (" << mvKeysRight[indices[i] + monoRight].pt.x << ", " << mvKeysRight[indices[i] + monoRight].pt.y << ")" << std::endl;
            continue;
        }

        if(dist1[i] < max_distance){
            //For every good match, check parallax and reprojection error to discard spurious matches
            Eigen::Vector3f p3D;
            descMatches++;
            int leftPos = i;
            int rightPos = indices[i];
            if(rightPos + monoRight >= mvKeysRight.size() || rightPos < 0){
                continue;
            }

            if(leftPos + monoLeft >= mvKeys.size() || leftPos < 0){
                continue;
            }

            float sigma1 = mvLevelSigma2[mvKeys[leftPos + monoLeft].octave], sigma2 = mvLevelSigma2[mvKeysRight[rightPos + monoRight].octave];
            float depth = static_cast<KannalaBrandt8*>(mpCamera)->TriangulateMatches(mpCamera2,mvKeys[leftPos + monoLeft],mvKeysRight[rightPos + monoRight],mRlr,mtlr,sigma1,sigma2,p3D);
            if(depth > 0.0001f) {
                mvLeftToRightMatch[leftPos + monoLeft] = rightPos + monoRight;
                mvRightToLeftMatch[rightPos + monoRight] = leftPos + monoLeft;
                mvStereo3Dpoints[leftPos + monoLeft] = p3D;
                mvDepth[leftPos + monoLeft] = depth;
                nMatches++;
            }
        }
    }

    std::cout << "BFmatcher DSP found : " << descMatches << " nMatches : " << nMatches << " good matches" << std::endl;

//    if(nMatches < 5){
//        std::cout << "BFmat : ALERT nMatches < 5, points : " << std::endl;
//        for(int i = 0; i < stereoDescLeft.rows; i++){
//            int leftPos = i;
//            int rightPos = indices[i];
//            int xl = mvKeys[leftPos + monoLeft].pt.x;
//            int yl = mvKeys[leftPos + monoLeft].pt.y;
//            int xr = mvKeysRight[rightPos + monoRight].pt.x;
//            int yr = mvKeysRight[rightPos + monoRight].pt.y;
//
//            std::cout << "BFmat : (" << xl << ", " << yl << ") <=> (" << xr << ", " << yr << "); " << std::endl;
//        }
//        std::cout << std::endl;
//    }
...
//        if((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7){ // TODO PUT BACK ORIGINAL : if((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7){
//            //For every good match, check parallax and reprojection error to discard spurious matches
//            Eigen::Vector3f p3D;
//            descMatches2++;
...
//                __android_log_print(ANDROID_LOG_INFO, "ORB-SLAM3", "[OPENCV GOOD]leftPos : %d (%f, %f) rightPos (%f, %f) : %d", (*it)[0].queryIdx, mvKeys[(*it)[0].queryIdx + monoLeft].pt.x, mvKeys[(*it)[0].queryIdx + monoLeft].pt.y, (*it)[0].trainIdx, mvKeysRight[(*it)[0].trainIdx + monoRight].pt.x, mvKeysRight[(*it)[0].trainIdx + monoRight].pt.y);
//                nMatches2++;
//            }
//            else __android_log_print(ANDROID_LOG_INFO, "ORB-SLAM3", "[OPENCV NOT GOOD]leftPos : %d (%f, %f) rightPos (%f, %f) : %d", (*it)[0].queryIdx, mvKeys[(*it)[0].queryIdx + monoLeft].pt.x, mvKeys[(*it)[0].queryIdx + monoLeft].pt.y, (*it)[0].trainIdx, mvKeysRight[(*it)[0].trainIdx + monoRight].pt.x, mvKeysRight[(*it)[0].trainIdx + monoRight].pt.y);
//
//        }
//    }
//    std::cout << "BFmatcher OpenCV found : " << descMatches2 << " nMatches : " << nMatches2 << " good matches" << std::endl;
}
