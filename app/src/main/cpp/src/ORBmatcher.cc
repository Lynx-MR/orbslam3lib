...
namespace ORB_SLAM3
{
    ...
    {
        while(!pKF1 || !pKF2){
            cout << "ALERT : pKF1 or pKF2 is NULL" << std::endl;
            usleep(100);
        }
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        std::mutex rotHistMutex;

        //Thread
        int nmatches = SearchByProjectionThread(CurrentFrame, LastFrame, th, bMono, 0, 1, rotHist, rotHistMutex);
    ...
    int ORBmatcher::SearchByProjectionThread(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono, int threadId, int nThreads, std::vector<int>* rotHist, std::mutex& rotHistMutex)
    {
        int nmatches = 0;
        ...
        for (int i = (threadId*LastFrame.N)/nThreads; i < ((threadId+1)*LastFrame.N)/nThreads; i ++) {
                    ...
                    if (bestDist <= TH_HIGH) {
                        ...
                        if (mbCheckOrientation) {
                            ...
                            if (bin == HISTO_LENGTH)
                                bin = 0;
                            assert(bin >= 0 && bin < HISTO_LENGTH);
                            //std::lock_guard<std::mutex> lock(rotHistMutex);
                    ...
                    if (CurrentFrame.Nleft != -1) {
                        ...
                        for (vector<size_t>::const_iterator vit = vIndices2.begin(), vend = vIndices2.end();
                            ...
                            if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft])
                                if (CurrentFrame.mvpMapPoints[i2 +
                                                              CurrentFrame.Nleft]->Observations() >
                                    0)
                                    continue;
                ...
                if(!pMP->isBad() && !sAlreadyFound.count(pMP))
                {
                    ...
                    if(bestDist<=ORBdist)
                    {
                        ...
                        if(mbCheckOrientation)
                        {
                            ...
                            if(bin==HISTO_LENGTH)
                                bin=0;
                            assert(bin>=0 && bin<HISTO_LENGTH);//TODO FIX THIS ERROR!!!
                            rotHist[bin].push_back(bestIdx2);
                        }
                    }
