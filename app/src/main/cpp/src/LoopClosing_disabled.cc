...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC, std::shared_ptr<std::mutex>& mutexLoopClosingLocalMapper):
   ...
   mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mpMatchedKF(NULL), mbLoopDetected(false), mnLoopNumCoincidences(0), mnLoopNumNotFound(0), 
    ...
    {
    mMutexLoopClosingLocalMapper = mutexLoopClosingLocalMapper; // REPRENDRE ICI, UTILISER CE MUTEX POUR PROTEGER LoopClosing/LocalMapper
...
{
    //DEBUG DISABLE LOOP CLOSING, PUT FALSE TO RENABLE IT
...
{
    unique_lock<mutex> lock(mMutexReset);
    //std::unique_lock<std::mutex> lockLoopClosing(*mMutexLoopClosingLocalMapper);
    if(mbResetRequested)
    {
        cout << "Loop closer reset requested..." << endl;
        //std::unique_lock<std::mutex> lockLocalMapping(*mMutexLoopClosingLocalMapper);
...
//            {
//                //unique_lock<mutex> lockLocalMapping(*mMutexLoopClosingLocalMapper);
